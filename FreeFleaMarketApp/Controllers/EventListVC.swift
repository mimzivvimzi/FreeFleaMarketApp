...
//        ref.child("posts").observeSingleEvent(of: .value, with: { (snapshot) in
            if let value = snapshot.value as? [String : [String : String?]] {
                for element in value.keys {
                    // when fetching a value from firebase, either use 1 or 2
                    let user = value[element]!["userID"]! //1 force unwrap it later in Event(user: user!) but app will crash if it is nil
                    let title = value[element]!["title"]! ?? "Untitled" //Give another option if it is nil but this is not advised in required fields
                    let fetchedEvent = Event(user: user!, title: title, date: value[element]!["date"]! ?? "", location: value[element]!["location"]! ?? "", image: nil, details: value[element]!["details"]! ?? "")
...
//        }
        ...
        //CHANGED THE DATE AND TIME FORMAT: STRING SPLITTING
        let event = eventList[indexPath.row]
        cell.eventTitle.text = event.title
        let dateTime = event.date.split(separator: " ") //"if you have this sentence" -> ["if", "you", "have", "this", "sentence"], 5 elements separated by space
        if dateTime.count != 0{
